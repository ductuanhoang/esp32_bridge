...
static esp_err_t file_get_handler(httpd_req_t *req) {
    ...
    // }
    if (file_name[strlen(file_name) - 1] == '/' && strlen(file_name) + strlen("login.html") < FILE_PATH_MAX) {
        strcpy(&file_name[strlen(file_name)], "login.html");
...
static httpd_handle_t web_server_start(void)
{
    ...
    config.max_uri_handlers = 15; // TODO: this is config max uri handlers
